...
class BridgeController extends Controller
{
    ...
    public function actionRun($id)
    {
        ...
        if ($model->bridge_type == 'independent') {
            ...
            // Load bridge column definitions early so we can detect patient-id source columns
            $bridgeColsForEntity = BridgeColumn::find()->where(['bridge_id' => $id])->all();
            $patientSourceColumns = [];
            foreach ($bridgeColsForEntity as $bcf) {
                $t = strtolower(trim($bcf->column_type ?? ''));
                if (preg_match('/^(patient[_\s-]?id|id)$/i', $t)) {
                    $patientSourceColumns[] = $bcf->source_column_name;
                }
            }
            ...
            $sourceIdToEntityId = [];
            $sourceToEntityMap = []; // [sourceCol][sourceValue] => entityId
            ...
            foreach ($RAW_DATA as $data) {
                ...
                // determine which original id to store in EntitySystem.entity_reference
                $entityReferenceValue = $data['id'];
                if (!empty($patientSourceColumns)) {
                    $psc = $patientSourceColumns[0];
                    if (isset($data[$psc]) && $data[$psc] !== '') {
                        $entityReferenceValue = $data[$psc];
                    }
                }
                ...
                // record mapping for patient source columns so we can replace old ids later
                foreach ($patientSourceColumns as $psc) {
                    if (isset($data[$psc])) {
                        $sourceToEntityMap[$psc][$data[$psc]] = $entityId;
                    }
                }
            }
            ...
            if (!empty($execute_list)) {
                ...
                foreach ($execute_list as $row) {
                    ...
                    foreach ($mapTargetToSource as $targetCol => $sourceCol) {
                            ...
                            // Accept variations like 'patient_id', 'patient id', 'patient-id' or plain 'id'
                            if (preg_match('/^(patient[_\s-]?id|id)$/i', $type)) {
                                // Prefer mapping by the source-column value (if available), otherwise fall back to primary-id mapping
                                $sourceVal = $row[$sourceCol] ?? null;
                                $mappedVal = null;
                                if ($sourceVal !== null && isset($sourceToEntityMap[$sourceCol][$sourceVal])) {
                                    $mappedVal = $sourceToEntityMap[$sourceCol][$sourceVal];
                                } elseif (isset($sourceIdToEntityId[$row['id']])) {
                                    $mappedVal = $sourceIdToEntityId[$row['id']];
                                }
                                $mapped[$targetCol] = $mappedVal;
                            } else {
